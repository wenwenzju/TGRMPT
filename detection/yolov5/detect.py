...
def run(weights=ROOT / 'yolov5s.pt',  # model.pt path(s)
    ...
    for ii,(path, im, im0s, vid_cap, s) in enumerate(dataset):
        ...
        for i, det in enumerate(pred):  # per image
            ...
            sub_path = '/'.join(p.split('/')[-4:-1])
            p = Path(p)  # to Path
            save_path = str(save_dir/  p.name)  # im.jpg
            txt_path = str(save_dir /sub_path / p.stem) + ('' if dataset.mode == 'image' else f'_{frame}')  # im.txt
            ...
            if len(det):
                # Rescale boxes from img_size to im0 sizekkkkkkkkkk
