...
def plot_compare_trackers(tracker_folder, tracker_list, cls, output_folder, plots_list=None, settings=None):
    ...
    for args in plots_list:
        create_comparison_plot(data, out_loc, *args, settings)
...
def create_comparison_plot(data, out_loc, y_label, x_label, sort_label, bg_label=None, bg_function=None, settings=None):
    ...
    labels = ["wb+hs", "wb", "hs"]
    plt.plot([x_values[0]], [y_values[0]], 'r.', markersize=15, label=labels[0])
    # plt.text(x_values[0], y_values[0], labels[0], color='red', fontsize=15)
    plt.plot([x_values[1]], [y_values[1]], 'g.', markersize=15, label=labels[1])
    # plt.text(x_values[1], y_values[1], labels[1], color='green', fontsize=15)
    plt.plot([x_values[2]], [y_values[2]], 'b.', markersize=15, label=labels[2])
    # plt.text(x_values[2], y_values[2], labels[2], color='blue', fontsize=15)
